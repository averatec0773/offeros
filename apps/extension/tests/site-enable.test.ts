...
/** Nobody answers a ping: there is no engine on this page yet. */
const noEngine = async () => {
...
describe("injecting the engine", () => {
  ...
  it("reports the page it refuses before touching the browser at all", async () => {
    const s = scripting();
    const res = await enableOnTab(1, "chrome://extensions", s, noEngine);
  ...
  it("surfaces a real injection failure verbatim", async () => {
    ...
    const res = await enableOnTab(1, "https://careers.example.com/apply", s, noEngine);
    expect(res.ok).toBe(false);
  ...
  it("says so when the browser has no scripting API rather than appearing to work", async () => {
    const res = await enableOnTab(1, "https://careers.example.com/apply", undefined, noEngine);
    expect(res.ok).toBe(false);
    expect(res.error).toBeTruthy();
  });

  it("succeeds quietly when both injections land", async () => {
    const res = await enableOnTab(1, "https://careers.example.com/apply", scripting(), noEngine);
    expect(res).toEqual({ ok: true });
  });
});

/**
 * The panel now asks for this on every page it opens on, so asking twice has to
 * be free. A second injection would register a second set of message handlers
 * and every scan would come back answered twice.
 */
describe("asking twice", () => {
  it("does not inject when the engine already answers", async () => {
    const s = scripting();
    const res = await enableOnTab(1, "https://careers.example.com/apply", s, async () => true);
    expect(res).toEqual({ ok: true });
    expect(s.executeScript).not.toHaveBeenCalled();
  });

  it("injects when nobody answers", async () => {
    const s = scripting();
    const res = await enableOnTab(1, "https://careers.example.com/apply", s, noEngine);
    expect(res).toEqual({ ok: true });
  ...
  it("does not ping a page it will not touch anyway", async () => {
    const ping = vi.fn(async () => true);
    const res = await enableOnTab(1, "chrome://extensions", scripting(), ping);
    expect(res.ok).toBe(false);
