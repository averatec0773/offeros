...
export function ApplicationDetailClient({
  ...
  // Filling again is only meaningful while the application is still open. Once
  // it has been sent (or closed), the header offers nothing to fill — the
  // "Original posting" link beside the title is the only way back to the page,
  // and it is deliberately quiet.
  const SENT: ApplicationStatus[] = ["applied", "interview", "offer", "rejected", "archived"];
  const showApplyButton = !SENT.includes(status);
  const canApply = Boolean(jobInfo.applyLink?.trim());
  const hasFilled = (task?.fieldReports?.length ?? 0) > 0;
      ...
      <header className="rounded-2xl border border-border bg-card p-5">
        <div className="flex flex-wrap items-start justify-between gap-4">
          ...
          <div className="flex flex-col items-end gap-2">
            {/* The one thing this page exists to help you do. It was buried in
                the left column under the job description; everything around it
                here is deliberately quieter so it reads as the main road. */}
            {showApplyButton && (
              <button
                type="button"
                ...
                title={
                  canApply
                    ? "Opens the posting in a new tab and hands it to the browser panel to fill"
                    : "No application link is saved for this job"
                }
                className="inline-flex items-center gap-2 rounded-full bg-primary px-5 py-2.5 text-body font-semibold text-primary-foreground press hover:bg-primary/85 disabled:cursor-not-allowed disabled:opacity-50"
              >
                <ExternalLink aria-hidden className="size-4" />
                {hasFilled ? "Re-fill this application" : "Open & fill this application"}
              </button>
            )}
            ...
            <div className="flex flex-wrap items-center justify-end gap-2">
              ...
              <Link
                ...
                className="inline-flex items-center gap-1.5 self-end rounded-full border border-border px-3.5 py-2 text-caption font-semibold text-foreground transition-colors hover:bg-muted"
              >
        ...
        {/* Said once, next to the button that does it. */}
        {showApplyButton && (
          <p className="mt-3 text-right text-caption text-muted-foreground">
            {canApply
              ? "Opens the posting and lets the browser panel fill it from your profile."
              : "No application link saved for this job — add one on the posting to fill it."}
          </p>
        )}
        {ticketCreated && (
