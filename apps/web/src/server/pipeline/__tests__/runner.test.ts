 ...
 *  an optional `throwAt` key makes that step's body throw. */
function makeSteps(
  opts: { requirement?: CoverLetterRequirement; throwAt?: string } = {},
): PipelineStep[] {
  return PIPELINE_STEPS.map(({ key }) => ({
    ...
    run: async (ctx, _task) => {
      ran.push(key);
      if (opts.throwAt === key) throw new Error(`boom at ${key}`);
